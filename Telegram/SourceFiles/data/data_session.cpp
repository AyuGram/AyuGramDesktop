	...
	if (!expired.empty()) {
		...
		if (settings.saveDeletedMessages()) {
			for (const auto &item : expired) {
				// remove from `_ttlMessages` to avoid re-processing infinitely
				item->applyTTL(0);
				// AyuGram: save the message instead of destroying it
				// (processMessageDelete destroys non-savable items itself)
