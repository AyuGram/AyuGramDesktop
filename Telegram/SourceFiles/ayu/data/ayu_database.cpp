...
namespace AyuDatabase {
	...
	std::vector<EditedMessage> getEditedMessages(ID userId, ID dialogId, ID messageId) {
		return storage.get_all<EditedMessage>(where(c(&EditedMessage::userId) == userId and
	...
	bool hasRevisions(ID userId, ID dialogId, ID messageId) {
		try {
			return storage.count<EditedMessage>(where(c(&EditedMessage::userId) == userId and
													  c(&EditedMessage::dialogId) == dialogId and
													  c(&EditedMessage::messageId) == messageId)) > 0;
		} catch (std::exception& ex) {
