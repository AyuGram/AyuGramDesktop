...
namespace AyuFeatures::MessageShot {
	...
	public:
		MessageShotDelegate(
			not_null<QWidget*> parent, not_null<Ui::ChatStyle*> st, Fn<void()> update, not_null<History*> history);
	...
	QImage addPadding(const QImage& original) {
		...
		QImage paddedImage(original.width() + 2 * st::messageShotPadding,
	...
	QImage Make(not_null<QWidget*> box, const ShotConfig& config) {
		...
		auto delegate = std::make_unique<MessageShotDelegate>(
			box, st.get(), [=] { box->update(); }, messages.front()->history());

		// remove deleted messages
		messages.erase(std::ranges::remove_if(messages,
											  [=](const auto& message) {
												  return !message ||
														 !controller->session().data().message(message->fullId());
											  })
						   .begin(),
					   messages.end());
			...
			if (displayUserpic) {
				...
				if (const auto from = message->displayFrom()) {
					...
					if (info->customUserpic.empty()) {
						...
						const auto valid = info->paintCustomUserpic(p, userpic, picX, picY, width, st::msgPhotoSize);
					}
				}
			}
