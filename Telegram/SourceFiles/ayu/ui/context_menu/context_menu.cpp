...
namespace AyuUi {
			...
			menu->addAction(
				...
				[=] {
					item->history()->session().tryResolveWindow()->showSection(
						std::make_shared<EditedLog::SectionMemento>(item->history()->peer, item));
				},
				...
				[=] {
					if (const auto window = item->history()->session().tryResolveWindow()) {
						if (const auto mainWidget = window->widget()->sessionController()) {
							...
							const auto key = (peer && !peer->isUser()) ? item->topic() ? Dialogs::Key{item->topic()}
	...
	void AddMessageDetailsAction(not_null<Ui::PopupMenu*> menu, HistoryItem* item) {
		...
		const auto messageForwards =
			views && views->forwardsCount > 0 ? QString::number(views->forwardsCount) : QString();
		...
		callback(Window::PeerMenuCallback::Args{
			...
			.fillSubmenu =
				[&](not_null<Ui::PopupMenu*> menu2) {
					...
					menu2->addAction(
						Ui::ContextActionWithSubText(menu2->menu(), st::menuIconInfo, QString("ID"), messageId));
						...
						if (isSticker) {
							...
							if (authorId != 0) {
								menu2->addAction(Ui::ContextActionStickerAuthor(
									menu2->menu(), &item->history()->session(), authorId));
							}
						}
					}
	...
	void AddReadUntilAction(not_null<Ui::PopupMenu*> menu, HistoryItem* item) {
		...
		menu->addAction(
			tr::ayu_ReadUntilMenuText(tr::now), [=]() { readHistory(item); }, &st::menuIconShowInChat);
	}
