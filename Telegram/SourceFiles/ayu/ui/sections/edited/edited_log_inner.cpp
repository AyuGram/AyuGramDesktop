...
namespace EditedLog {
	...
	InnerWidget::InnerWidget(QWidget* parent,
		...
		Window::ChatThemeValueFromPeer(controller, peer) | rpl::start_with_next(
		...
		session().data().viewRepaintRequest() | rpl::start_with_next(
													[=](auto view) {
													...
													lifetime());
		session().data().viewResizeRequest() | rpl::start_with_next(
												   [=](auto view) {
													   if (view->delegate() == this) {
														   resizeItem(view);
													   }
												   ...
												   lifetime());
		session().data().itemViewRefreshRequest() | rpl::start_with_next(
														[=](auto item) {
														...
														lifetime());
		session().data().viewLayoutChanged() | rpl::start_with_next(
												   [=](auto view) {
													   if (view->delegate() == this) {
												   ...
												   lifetime());
		session().data().itemDataChanges() | rpl::start_with_next(
		...
		session().data().itemVisibilityQueries() | rpl::filter([=](const Data::Session::ItemVisibilityQuery& query) {
			return (_history == query.item->history()) && query.item->isAdminLogEntry() && isVisible();
		}) |
			rpl::start_with_next(
		...
		controller->adaptive().chatWideValue() |
			rpl::start_with_next([=](bool wide) { _isChatWide = wide; }, lifetime());
	...
	void InnerWidget::updateEmptyText() {
		...
		auto text = Ui::Text::Semibold((hasSearch || hasFilter) ? tr::lng_admin_log_no_results_title(tr::now)
																: tr::lng_admin_log_no_events_title(tr::now));
		auto description = _peer->isMegagroup() ? tr::lng_admin_log_no_events_text(tr::now)
	...
	void InnerWidget::saveState(not_null<SectionMemento*> memento) {
		if (!_filterChanged) {
			...
			memento->setItems(base::take(_items), base::take(_eventIds), _upLoaded, _downLoaded);
	...
	void InnerWidget::itemsAdded(Direction direction, int addedCount) {
		...
		auto checkFrom = (direction == Direction::Up) ? (_items.size() - addedCount)
	...
	void InnerWidget::showContextMenu(QContextMenuEvent* e, bool showFromTouch) {
		...
		if (lnkPhoto || lnkDocument) {
			if (isUponSelected > 0) {
				_menu->addAction(
					tr::lng_context_copy_selected(tr::now), [=] { copySelectedText(); }, &st::menuIconCopy);
			}
			if (lnkPhoto) {
			...
			if (isUponSelected > 0) {
				_menu->addAction(
					tr::lng_context_copy_selected(tr::now), [this] { copySelectedText(); }, &st::menuIconCopy);
			} else {
	...
	void InnerWidget::updateSelected() {
		...
		auto point = QPoint(std::clamp(mousePosition.x(), 0, width()),
