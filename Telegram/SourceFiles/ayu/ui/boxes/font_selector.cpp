...
namespace AyuUi {
	...
	Rows::Rows(QWidget* parent, const std::vector<Font>& data, const QString& chosen) :
	...
	void Rows::mouseReleaseEvent(QMouseEvent* e) {
		...
		if (pressed == _selected) {
			v::match(
				pressed, [&](RowSelection data) { activateByIndex(data.index); }, [](v::null_t) {});
		}
	}
	...
	int Rows::indexFromSelection(Selection selected) const {
		return v::match(
			selected, [&](RowSelection data) { return data.index; }, [](v::null_t) { return -1; });
	}
	...
	void Rows::repaint(Selection selected) {
		v::match(
			selected, [](v::null_t) {}, [&](const auto& data) { repaint(data.index); });
	}
	...
	int Rows::countAvailableWidth(int newWidth) const {
		const auto right = 0;
		return newWidth - st::passportRowPadding.left() - st::langsRadio.diameter - st::passportRowPadding.left() -
			   right - st::passportRowIconSkip;
	}
	...
	Content::Content(QWidget* parent, const std::vector<Font>& fonts) : RpWidget(parent) {
		setupContent(fonts);
	}
...
AyuUi::FontSelectorBox::prepare() {
	...
	inner->activations() | rpl::start_with_next(
							   [=](const Font& font) {
								   if (inner) {
...
void
AyuUi::FontSelectorBox::setupTop(not_null<Ui::VerticalLayout*> container) {
	if (!_controller) {
		return;
	}
}
...
void
AyuUi::FontSelectorBox::keyPressEvent(QKeyEvent* e) {
...
int
AyuUi::FontSelectorBox::rowsInPage() const {
	return std::max(height() / AyuUi::Rows::DefaultRowHeight(), 1);
}

void
AyuUi::FontSelectorBox::setInnerFocus() {
	_setInnerFocus();
}
